...
def main(args):
  ...
  if not load:
    ...
    br.read("/loc/vive/trackers", librvsolve.trackerhandler, trackers)
    # Reading light data
    data = [np.empty([0,3]),
      np.empty([0,1]),
      np.empty([0,3]),
      np.empty([0,1])]
    lr = librvsolve.LightReader(trainfile, trackers)
    ...
    # Saving the data
    if save:
      dumpfile = open("tmp/svr_models2.bin", "wb")
      pickle.dump(models, dumpfile)
      dumpfile.close()
  else:
    # Saving the data
    loadfile = open("tmp/svr_models2.bin", "rb")
  ...
  data = [np.empty([0,3]),
    ...
    np.empty([0,3]),
    np.empty([0,1])]
  lr = librvsolve.LightReader(testfile, trackers)
  lr.read("/loc/vive/light", data)
